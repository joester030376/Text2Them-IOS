...
{
    func GetHandler(Token:String,completion:@escaping (GetCountryModel?, Error?)-> Void) {
    ...
    //    let param = APIParameter.token(grant_type: <#String#>, UserName: UserName, Password: Password).dictionary()
      ...
      //  let param = APIParameter.forgotPasswordSentMail(Email: UserEmail).dictionary()
        ...
        let param = APIParameter.GetCountry(Token: Token).dictionary()
        APIManager.callApi(API.GetCountry.requestString(), param: param, method:.post, header: nil,encodeType: URLEncoding.default, isLoader: true) { (code, error, data) in
            ...
            if data != nil
            {
                ...
                do {
                    if let `data` = data {
                        let ScheduleModels = try JSONDecoder().decode(GetCountryModel.self, from: data as! Data)
